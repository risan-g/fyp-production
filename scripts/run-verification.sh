...
export DOTWV_PUBLIC_SUPABASE_URL=$(cd .. && npx supabase status -o env | grep API_URL | cut -d '=' -f 2 | tr -d '"' | tr -d '\r')
export DOTWV_PUBLIC_SUPABASE_ANON_KEY=$(cd .. && npx supabase status -o env | grep ANON_KEY | cut -d '=' -f 2 | tr -d '"' | tr -d '\r')
